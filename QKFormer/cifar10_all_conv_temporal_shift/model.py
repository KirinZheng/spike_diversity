...
class TemporalConstruction(nn.Module):
    def __init__(self, n_segment=3, n_div=8, inplace=False):
        super(TemporalConstruction, self).__init__()
    ...
    def shift(x, n_segment, fold_div=3, inplace=False):
        T, n_batch, C, H, W = x.size()
        # nt, c, h, w = x.size()
        # n_batch = nt // n_segment
        assert T == n_segment, f"The T dimension of x must be as the same as n_segment, while T is {T} and n_segment is {n_segment}"
        ...
        out = out.transpose(0, 1).contiguous()  # T B C H W
        return out


class Token_QK_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,
                 temporal_construction_factor=1.0, feature_dimension_group_number=3, time_step=1):
        super().__init__()
        ...
        self.temporal_construction_factor = temporal_construction_factor
        self.temporal_construction = TemporalConstruction(n_segment=time_step, n_div=feature_dimension_group_number, inplace=False)


    def forward(self, x):
        T, B, C, H, W = x.shape

        x_temporal = self.temporal_construction(x)
        x = x + self.temporal_construction_factor * x_temporal
...
class Spiking_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,
                 feature_dimension_group_number=3, temporal_construction_factor=1.0, time_step=1):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
        self.dim = dim
        self.num_heads = num_heads
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        # only the first MLP has temporal construction ?
        x_temporal = self.temporal_construction(x)
        x = x + self.temporal_construction_factor * x_temporal
...
class TokenSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 feature_dimension_group_number=3, temporal_construction_factor=1.0, time_step=1):
        super().__init__()
        self.tssa = Token_QK_Attention(dim, num_heads, feature_dimension_group_number=feature_dimension_group_number,
                                       temporal_construction_factor=temporal_construction_factor, time_step=time_step)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features= dim, hidden_features=mlp_hidden_dim, drop=drop, 
                       feature_dimension_group_number=feature_dimension_group_number,
                       temporal_construction_factor=temporal_construction_factor, time_step=time_step)
...
class SpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 feature_dimension_group_number=3, temporal_construction_factor=1.0, time_step=1):
        super().__init__()
        self.ssa = Spiking_Self_Attention(dim, num_heads, feature_dimension_group_number=feature_dimension_group_number,
                                          temporal_construction_factor=temporal_construction_factor, time_step=time_step)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features= dim, hidden_features=mlp_hidden_dim, drop=drop,
                       feature_dimension_group_number=feature_dimension_group_number,
...
class PatchEmbedInit(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256,
        ...
        self.temporal_construction = TemporalConstruction(n_segment=time_step, n_div=feature_dimension_group_number, inplace=False)
        self.feature_dimension_group_number = feature_dimension_group_number
        self.temporal_construction_factor = temporal_construction_factor

        # self.temporal_construction_residual = TemporalConstruction(n_segment=time_step, n_div=feature_dimension_group_number, inplace=False)
    ...
    def forward(self, x):
        ...
        x_feat_temporal = self.temporal_construction(x_feat)
        x_feat = x_feat + self.temporal_construction_factor * x_feat_temporal
...
class PatchEmbeddingStage(nn.Module):
    ...
    def forward(self, x):
        ...
        x_temporal_2 = self.temporal_construction(x)
        x = x + self.temporal_construction_factor * x_temporal_2
        x = x.flatten(0, 1).contiguous()
...
class spiking_transformer(nn.Module):
    def __init__(self,
                 ...
                 feature_dimension_group_number = 3,
                 temporal_construction_factor = 1.0
                 ):
        super().__init__()
        ...
        patch_embed1 = PatchEmbedInit(img_size_h=img_size_h,
                                       ...
                                       feature_dimension_group_number=feature_dimension_group_number,
                                       temporal_construction_factor=temporal_construction_factor,
                                       time_step=T)
        ...
        stage1 = nn.ModuleList([TokenSpikingTransformer(
            ...
            feature_dimension_group_number=feature_dimension_group_number,
            temporal_construction_factor=temporal_construction_factor, time_step=T)
            for j in range(1)])
...
def QKFormer(pretrained=False, **kwargs):
    ...
    print(f"feature_dimension_group_number: {model.feature_dimension_group_number}")
    print(f"temporal_construction_factor: {model.temporal_construction_factor}")
    return model
