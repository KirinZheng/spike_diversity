...
def parse_args():
    ...
    parser.add_argument('--data-path', default='/zhengzeqi/dataset/cifar10dvs', help='dataset')
...
def load_data(dataset_dir, distributed, T, cache_dir):

    # 2025-06-17
    train_set_pth = os.path.join(cache_dir, f'cifar10dvs_train_set_{T}_size_128.pt')
    test_set_pth = os.path.join(cache_dir, f'cifar10dvs_test_set_{T}_size_128.pt')
...
def main(args):
    ...
    dataset_train, dataset_test, train_sampler, test_sampler = load_data(data_path, args.distributed, args.T, args.dts_cache)
