...
def load_model_noopt(args, model_without_ddp):
    if args.resume:
        print("misc load_model_noopt")
        if args.resume.startswith('https'):
            checkpoint = torch.hub.load_state_dict_from_url(
                args.resume, map_location='cpu', check_hash=True)
        else:
        ...
        # pretrain_dict = {k:v for k,v in pretrain_dict.items() if k in model_dict and k != 'patch_embed1.learnable_pos_embed'}
