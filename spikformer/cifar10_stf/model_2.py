...
class SSA(nn.Module):
    ...
    def forward(self, x, x_v=None):
        if isinstance(x, torch.Tensor):
            ...
            if x_v is not None:
                x_v = x_v.flatten(-2).transpose(-1, -2).flatten(0, 1)
                v_linear_out = self.v_linear(x_v)
            else:
    ...
    def forward_features(self, x, use_dense_connection=False):
        ...
        if self.dense_connection:
            ...
            if use_dense_connection:
                ...
                x_v = None      # changed on 2025-09-26
                for rep_idx in range(1, self.n_repeat + 1):
                    for i in range(self.increate_T_every):
                        x = block[(rep_idx - 1) * self.increate_T_every  + i](x, x_v=x_v)
...
@register_model
def spikformer_2(pretrained=False, **kwargs):
    model = Spikformer(
