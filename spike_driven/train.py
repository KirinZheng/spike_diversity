...
from dvs_utils import build_dataset 
import torch.distributed as dist
import module.misc as misc
...
def finetune_checkpoint(
    ...
    if os.path.isfile(checkpoint_path):
        ...
        if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
            if log_info:
                _logger.info('Finetune model state from checkpoint...')
...
parser.add_argument('--spike_re_eval', action='store_true', default=False)
parser.add_argument('--spike_hook_layer_name', type=str, default=None, help="hook layer name")
parser.add_argument('--spike_re_save_pth', type=str, default=False)
...
parser.add_argument(
    "--finetune", default="", 
    type=str, metavar="PATH", help="Resume full model except cls_head from checkpoint (default: none)",
    )

## changed on 2025-07-15
...
def main():
    ...
    if not args.use_imp_lif:                  # changed on 2025-05-01
        ...
        model = create_model(
            ...
            diversity_loss=args.use_dr_entropy_loss, # changed on 2025-04-24
            lif_recurrent_state=args.lif_recurrent_state, # changed on 2025-04-27
            use_imp_lif=args.use_imp_lif       # changed on 2025-04-27
        )
    ...
    # optionally resume from a finetune checkpoint
    if args.finetune:
        resume_epoch = finetune_checkpoint(
            model,
            args.finetune,
            log_info=args.local_rank == 0,
        )
    ...
    #     lr_scheduler.step(i)
    #     print(f"index:{i} lr:{lr_scheduler._get_lr(i)}")
    # assert False
    ##########################################################
    start_epoch = 0
    ...
    if args.dataset == "cifar10-dvs-tet":
        ...
        # changed on 2025-06-17
        train_set_pth = os.path.join(args.dts_cache, f'cifar10dvs_train_set_{args.time_steps}_size_64.pt')
        test_set_pth = os.path.join(args.dts_cache, f'cifar10dvs_test_set_{args.time_steps}_size_64.pt')

        if os.path.exists(train_set_pth) and os.path.exists(test_set_pth):
            print("Data have been splitted, now just loading from .pt files")
            st = time.time()
            dataset_train = torch.load(train_set_pth)
            dataset_eval = torch.load(test_set_pth)
            ...
            torch.save(dataset_eval, test_set_pth)
    ...
    # changed on 2025-04-23, 我猜测是因为create_dataset是对imagenet定制的
    ...
    if args.dataset in dvs_utils.DVS_DATASET:
        ...
        print(type(data_config["input_size"]))
    ...
    # changed on 2025-03-18, just statistic spike representation
    if args.spike_re_eval:
        if "tiny_imagenet" in args.dataset_name:
            functional.reset_net(model)
            eval_metrics = validate_spike_re(
                model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
            merge_saved_spikes(args=args)
            exit(0)
        ...
        # 仅限制于小数据集合上
            ...
            if not args.recurrent_coding:
                # hook取layer之后的输出
                def direct_coding_hook(module, inp, out):
                    if len(out.shape) == 4:
                        T_B, C, H, W = out.shape
                        out_save = out.reshape(args.time_steps, -1, C, H, W)  # T B C H W
                        hook_output.append(out_save.data.cpu().numpy())
                    else:
                    ...
                    # assert False
                    # 保存LIF电压序列
                    voltage_sequence.append(module.v_seq.data.cpu().numpy())
            
                for name, layer in model.named_modules():
                    # if name == "patch_embed.proj_lif":  # 确保名称匹配
                    if name == args.spike_hook_layer_name:  # 确保名称匹配 
                    # if name == "patch_embed1.proj_bn":  # 确保名称匹配
                        layer.register_forward_hook(direct_coding_hook)
                        print(f"Hook registered on: {name}")
            else:
                # hook 取layer之前的输入
                def direct_coding_pre_hook(module, inp):
                    T_B, C, H, W = inp[0].shape
                    inp_save = inp[0].reshape(args.time_steps, -1, C, H, W)  # T B C H W
                    hook_output.append(inp_save.data.cpu().numpy())
            
                for name, layer in model.named_modules():
                    # if name == "patch_embed.proj_conv1":  # 确保名称匹配
                    if name == args.spike_hook_layer_name:  # 确保名称匹配 
                    # if name == "patch_embed1.maxpool":  # 确保名称匹配
                        layer.register_forward_pre_hook(direct_coding_pre_hook)
                        print(f"Hook registered on: {name} pre hook")


            
            if args.dataset == "ncaltech101":
                additional_aug = transforms.Compose([transforms.Resize((128, 128))])

            last_idx = len(loader_eval) - 1
            with torch.no_grad():
                for batch_idx, (input, target) in enumerate(loader_eval):
                    original_pic.append(input.data.cpu().numpy())
                    input = input.float()
                    # changed on 2025-01-21
                    try:
                        N,_,_,_,_ = input.shape
                    except:
                        N,_,_,_ = input.shape

                    if args.dataset == "ncaltech101":
                        input = torch.stack([(additional_aug(input[i])) for i in range(N)])
                ...
                # 查看direct coding的统计
                if not args.recurrent_coding:
                    stacked_output = np.concatenate(hook_output, axis=1) # T B C H W
                else:
                    stacked_output = np.concatenate(hook_output, axis=1) # T B C H W
                original_pic_input = np.concatenate(original_pic, axis=0) # B C H W
                # np.savez('/zhengzeqi/top_down/QKFormer_original/cifar10_2/spike_result/baseline_first_batchnorm_after.npz', original_pic=original_pic_input, spike_representation=stacked_output)
                # np.savez('/zhengzeqi/top_down/QKFormer_original/cifar10_2/spike_result/baseline_first_batchnorm_before.npz', original_pic=original_pic_input, spike_representation=stacked_output)
                # assert False
                stacked_output = stacked_output.astype(np.uint8)  # 减少内存占用
                binary_data = stacked_output.reshape(args.time_steps, -1).T   # (N*48*32*32, time_step)
                encoded = np.dot(binary_data, 2**np.arange(args.time_steps - 1, -1, -1))  # 4-bit 组合编码为 0-15
                combination_counts = np.bincount(encoded, minlength=16)

                total_count = len(encoded)
                for i in range(2 ** args.time_steps):
                    comb = tuple(map(int, format(i, f'0{args.time_steps}b')))
                    print(f"Combination: {comb}, Count: {combination_counts[i]}, Ratio: {combination_counts[i]/total_count:.6f}")
                
                positive_rate = np.mean(binary_data)
                print(f"Firing rate: {positive_rate:.6f}")
...
###########################################changed on 2025-03-20 ###############################################

def direct_code_entropy_loss(spike_patterns, T=4, mode="global", normalize=True):
    """
    计算 Direct Coding 熵正则项（支持全局或逐样本计算）
    
    :param spike_patterns: Tensor，形状为 (T, B, C, H, W)
    :param T: 时间步长
    :param mode: "global" 或 "per_sample"
    :return: 标量损失值
    """
    T, B, C, H, W = spike_patterns.shape
    # spike_patterns = spike_patterns.reshape(T, -1, C, H, W)  # T B C H W
    # T, B, C, H, W = spike_patterns.shape

    if mode == "global":
        patterns = spike_patterns.reshape(T, -1).T  # 所有样本拼一起 → (N, T)
        unique_patterns, counts = torch.unique(patterns, return_counts=True, dim=0)
        probs = counts.float() / counts.sum()
        entropy = -torch.sum(probs * torch.log(probs + 1e-10))
        if normalize:
            entropy = entropy / torch.log(torch.tensor(2**T))
        return -entropy / B  # 平均分配到每个样本
        

    elif mode == "per_sample":
        losses = []
        for b in range(B):
            sample = spike_patterns[:, b, :, :, :]  # (T, C, H, W)
            flattened = sample.reshape(T, -1).T     # (N, T)
            unique_patterns, counts = torch.unique(flattened, return_counts=True, dim=0)
            probs = counts.float() / counts.sum()
            entropy = -torch.sum(probs * torch.log(probs + 1e-10))
            if normalize:
                entropy = entropy / torch.log(torch.tensor(2**T))
        ...
        raise ValueError(f"Unsupported mode: {mode}. Choose 'global' or 'per_sample'.")
...
def merge_saved_spikes(args):
    if not misc.is_main_process():
        return

    save_dir = os.path.join(args.spike_re_save_pth)
    files = sorted(glob.glob(os.path.join(save_dir, f"{args.dataset_name}_T_{args.time_steps}_{args.pe_type}_epoch_{args.start_epoch-1}_rank*.npz")))
    ...
    for f in files:
        data = np.load(f)
        all_spikes.append(data["spike"])
        os.remove(f)  # 删除中间文件
        print(f"Deleted: {f}")

    merged = np.concatenate(all_spikes, axis=1)  # B 维拼接, T B C H W, np.uint8


    binary_data = merged.reshape(args.time_steps, -1).T   # (BCHW, time_step)
    encoded = np.dot(binary_data, 2**np.arange(args.time_steps - 1, -1, -1))  # time_step-bit 组合编码为 0- (2**time_step-1)
    combination_counts = np.bincount(encoded, minlength=2**args.time_steps)

    total_count = len(encoded)
    combination_ratios = combination_counts / total_count
    ...
    # 保存结果为 .npz
    save_path = os.path.join(save_dir, f"spike_representation_{args.dataset_name}_T_{args.time_steps}_{args.pe_type}_epoch_{args.start_epoch-1}.npz")
    np.savez(save_path,
            #  spike=merged.astype(np.uint8),
             combination_counts=combination_counts,
             combination_ratios=combination_ratios,
             firing_rate=np.array(positive_rate, dtype=np.float32))
    

def validate_spike_re(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=""):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
    ...
    def save_spike_hook(input_tensor):
        # 自动创建目录（如果不存在）
        os.makedirs(args.spike_re_save_pth, exist_ok=True) 

        spike = input_tensor.cpu().numpy().astype(np.uint8)
        save_path = os.path.join(args.spike_re_save_pth, f"{args.dataset_name}_T_{args.time_steps}_{args.pe_type}_epoch_{args.start_epoch-1}_rank{rank}_batch{save_counter['count']}.npz")
        np.savez(save_path, spike=spike)
    ...
    if not args.recurrent_coding:
        def direct_coding_hook(module, inp, out):
            if len(out.shape) == 4:
                T_B, C, H, W = out.shape
                out_save = out.reshape(args.time_steps, -1, C, H, W)
                save_spike_hook(out_save)
            else:
                save_spike_hook(out)
            # voltage_sequence.append(module.v_seq.cpu().numpy())
        
        for name, layer in actual_model.named_modules():
            if name == "patch_embed.proj_lif":
                layer.register_forward_hook(direct_coding_hook)
                print(f"Hook registered on: {name}")
    else:
        def direct_coding_pre_hook(module, inp):
            T_B, C, H, W = inp[0].shape
            inp_save = inp[0].reshape(args.time_steps, -1, C, H, W)
            save_spike_hook(inp_save)

        for name, layer in actual_model.named_modules():
            if name == "patch_embed.proj_conv1":
                layer.register_forward_pre_hook(direct_coding_pre_hook)
                print(f"Hook registered on: {name} pre hook")
